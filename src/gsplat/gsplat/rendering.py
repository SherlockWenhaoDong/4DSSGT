...
def rasterization(
    ...
    if render_mode in ["RGB+D", "RGB+ED"]:
        ...
        if backgrounds is not None:
            target_C = colors.shape[-1]
            if backgrounds.shape[-1] > target_C:
                backgrounds = backgrounds[..., :target_C]
            elif backgrounds.shape[-1] < target_C:
                pad = target_C - backgrounds.shape[-1]
...
def _rasterization(
    ...
    return render_colors.contiguous().clone(), render_alphas.contiguous().clone(), meta.contiguous().clone()
...
def rasterization_rgb_semantic(
    means: Tensor,
    quats: Tensor,
    scales: Tensor,
    opacities: Tensor,
    ...
    far_plane: float = 1e10,
    backgrounds: Optional[Tensor] = None,
    render_mode: Literal["RGB", "RGB+ED", "RGB+D"] = "RGB+ED",
    **kwargs,
):
    """
    ✅ RGB + Semantic Joint Rendering
    ...
    output:
        render_rgb: [..., C, H, W, 3]
        render_semantic: [..., C, H, W, S]
        render_alpha: [..., C, H, W, 1]
        render_depth: [..., C, H, W, 1]
    """
    C = viewmats.shape[-3]
    ...
    if backgrounds is None:
        backgrounds = torch.zeros(
            (C, height, width, target_C),
            device=colors.device,
            dtype=colors.dtype,
        )
    else:
        if backgrounds.shape[-1] != target_C:
            diff = target_C - backgrounds.shape[-1]

            if diff > 0:
                pad = torch.zeros(
                    (*backgrounds.shape[:-1], diff),
                    device=backgrounds.device,
                    dtype=backgrounds.dtype,
                )
                backgrounds = torch.cat([backgrounds, pad], dim=-1)

            else:
                backgrounds = backgrounds[..., :target_C]

    render_all, render_alpha, meta = rasterization(
        means=means,
        quats=quats,
        scales=scales,
        opacities=opacities,
        colors=colors,                 # ✅ multi-channel
        viewmats=viewmats,
        Ks=Ks,
        width=width,
        height=height,
    ...
    if "D" in render_mode:
        render_depth = render_all[..., -1:]       # [..., H, W, 1]
        render_feat = render_all[..., :-1]        # [..., H, W, D]
    else:
        render_depth = torch.zeros_like(render_alpha)
        render_feat = render_all

    render_rgb = render_feat[..., :3]
    render_semantic = (
        render_feat[..., 3:] if render_feat.shape[-1] > 3 else None
    )

    return render_rgb.contiguous().clone(), render_semantic.contiguous().clone(), render_alpha.contiguous().clone(), render_depth.contiguous().clone(), meta
