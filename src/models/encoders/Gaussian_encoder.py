...
from src.models.acceleration.checkpoint import auto_grad_checkpoint
from src.models.geometry import dt_to_cov_t, radius_to_sigma
from src.models.encoders.image_encoder import BaseImageEncoder
from src.models.blocks import get_layernorm, SelfAttentionBlock, build_pytorch_mlp
...
class ImageEncoder4DG(BaseImageEncoder):
    def __init__(
        ...
        self.dino = timm.create_model(
            ...
            pretrained=False,  # 关键修改
            num_classes=0,
        )
        self.dino_embed_dim = 768

    def load_4dgt_stage1(
            self,
            ckpt_path,
    ):
        ckpt = torch.load(
    ...
    def extract_dino_features(
        ...
        self.grid_h = H // self.patch_size[0]
        self.grid_w = W // self.patch_size[1]

        feats = patch_tokens.reshape(
            B,
            T,
            N,
    ...
    def decode2(
        ...
        depth = self.unpatchify(
            self.mlp_depth(token),
            1,
            H,
            W,
        ).permute(0, 2, 3, 4, 1)
        ...
        rgb = self.unpatchify(
            self.mlp_rgb(app_token),
            self.color_dim,
            H,
            W,
        ).permute(0, 2, 3, 4, 1)

        opacity = self.unpatchify(
            self.mlp_opacity(token),
            1,
            H,
            W,
        ).permute(0, 2, 3, 4, 1)

        scale = self.unpatchify(
            self.mlp_scale(token),
            3,
            H,
            W,
        ).permute(0, 2, 3, 4, 1)

        rotation = self.unpatchify(
            self.mlp_rotation(token),
            4,
            H,
            W,
        ).permute(0, 2, 3, 4, 1)

        dxyzt = self.unpatchify(
            self.mlp_dxyzt(token),
            4,
            H,
            W,
        ).permute(0, 2, 3, 4, 1)
        ...
        cov_t = self.unpatchify(
            self.mlp_cov_t(token),
            1,
            H,
            W,
        ).permute(0, 2, 3, 4, 1)
        ...
        target_h = depth.shape[2]
        target_w = depth.shape[3]
        ray_o = ray_o.permute(
            0, 2, 3, 4, 1
        )
        ...
        gs_params["depth"] = depth
        gs_params["feature"] = rgb
        gs_params["opacity"] = opacity
        gs_params["scaling"] = scale
        gs_params["rotation"] = rotation


        gs_params["xyz"] = (
                gs_params["depth"] * ray_d
                + ray_o
        )
        ...
        gs_params["ms3"] = ms3
        gs_params["cov_t"] = cov_t

        gs_params["dxyzt"] = dxyzt
        ...
        for k in gs_params:
            gs_params[k] = gs_params[k].reshape(
                B,
                T * H * W,
                -1
            ).float()
    ...
    def forward(
        ...
        dino_tokens = self.extract_dino_features(
            rgb_frames
        )
        # [B,T,N,768]
    ...
    def unpatchify(
        ...
        x = rearrange(
            x,
            "B (N_t N_h N_w) (C_out H_p W_p) -> \
             B C_out N_t (N_h H_p) (N_w W_p)",
            N_t=N_t,
            N_h=N_h,
            N_w=N_w,
            H_p=Hp,
