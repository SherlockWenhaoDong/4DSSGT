...
def rasterize_to_pixels(
    ...
    if channels not in (
        ...
        if backgrounds is not None:
            if backgrounds.shape[-1] < colors.shape[-1]:
                bg_pad = colors.shape[-1] - backgrounds.shape[-1]
            ...
            elif backgrounds.shape[-1] > colors.shape[-1]:
                backgrounds = backgrounds[..., :colors.shape[-1]]

    else:
        padded_channels = 0


    # assert backgrounds.shape == image_dims + (colors.shape[-1],), backgrounds.shape
